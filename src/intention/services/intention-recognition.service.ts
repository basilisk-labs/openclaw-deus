    ...
    properties: {
      new_intentions: {
        ...
        items: {
          ...
          properties: {
            ...
            kind: { type: 'string' as const, enum: ['goal', 'task', 'sub_task', 'standing_order'] },
            source: { type: 'string' as const, enum: ['operator_explicit', 'operator_inferred'] },
            success_criteria: { type: 'string' as const },
            parent_intention_id: { type: 'string' as const },
            priority: { type: 'number' as const },
            reasoning: { type: 'string' as const },
          },
          required: ['description', 'kind', 'source', 'success_criteria', 'priority', 'reasoning'],
        },
      },
      updated_intentions: {
        type: 'array' as const,
      ...
      completed_intentions: {
        ...
        items: {
          ...
          properties: {
            intention_id: { type: 'string' as const },
            outcome: { type: 'string' as const, enum: ['completed', 'failed', 'abandoned'] },
            reasoning: { type: 'string' as const },
          },
...
export class IntentionRecognitionService {
   ...
   */
  async recognizeFromMessage(message: string): Promise<Result<IntentionRecognitionResult, DomainError>> {
    ...
    if (message.trim().length < 10) {
      return ok({ new_intentions: [], updated_intentions: [], completed_intentions: [] });
    }
    ...
    const result = await this.llm.call<IntentionRecognitionResult>({
      ...
      forceTool: 'update_intentions',
    });

    if (result.isErr()) {
      this.logger.warn(`LLM intention recognition failed, using fallback: ${result.error.message}`);
      return this.fallbackRecognition(message, activeIntentions);
    }

    const recognition = result.value.data;
  ...
  private async applyRecognition(recognition: IntentionRecognitionResult): Promise<void> {
    ...
    for (const newInt of recognition.new_intentions) {
      ...
      await this.intentions.create({
        ...
        progress: { estimated_completion: 0, last_action: 'just recognized', blockers: [] },
   ...
   */
  private fallbackRecognition(message: string, active: Intention[]): Result<IntentionRecognitionResult, DomainError> {
    const lower = message.toLowerCase();
    const result: IntentionRecognitionResult = { new_intentions: [], updated_intentions: [], completed_intentions: [] };
    ...
    for (const pattern of taskPatterns) {
      ...
      if (match && match[1] && match[1].length > 10) {
        result.new_intentions.push({
          ...
          priority: 0.7,
          reasoning: 'Detected explicit task request via pattern matching (LLM unavailable)',
        });
    ...
    const completionSignals = ['done', 'finished', 'complete', 'готово', 'сделано', 'хватит', 'достаточно'];
  ...
  private buildUserMessage(message: string, activeIntentions: Intention[]): string {
    const intentionsContext = activeIntentions.length > 0
