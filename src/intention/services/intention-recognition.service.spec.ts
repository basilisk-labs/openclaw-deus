...
describe("IntentionRecognitionService", () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: SimilarityProvider,
          useValue: {
            findBestWordMatch: jest.fn().mockReturnValue(null),
            wordOverlap: jest.fn().mockReturnValue(0),
          },
        },
      ],
    }).compile();
  ...
  describe("recognizeFromMessage (fallback mode)", () => {
    ...
    it("should detect explicit task requests in English", async () => {
      const result = await service.recognizeFromMessage(
        "Can you fix the authentication bug in the login page?",
      );
      expect(result.isOk()).toBe(true);
    ...
    it("should detect explicit task requests in Russian", async () => {
      const result = await service.recognizeFromMessage(
        "Давай напиши тесты для модуля авторизации",
      );
      expect(result.isOk()).toBe(true);
      expect(result._unsafeUnwrap().new_intentions.length).toBeGreaterThan(0);
    });
    ...
    it("should detect completion signals", async () => {
      ...
      intentionService.findActive.mockResolvedValue(
        ok([
      ...
      const result = await service.recognizeFromMessage(
        "Done, the fix is ready",
      );
      expect(result.isOk()).toBe(true);
      expect(
        result._unsafeUnwrap().completed_intentions.length,
      ).toBeGreaterThan(0);
    });
  });
});
