...
import { CognitiveSnapshot, DimensionName, DIMENSION_THRESHOLDS } from './metrics.types';
import { Diagnosis, DiagnosisResult, DiagnosisSeverity, Hypothesis, RootCause, ParamChange } from './diagnosis.types';
...
const DIAGNOSIS_TOOL = {
  name: 'diagnose',
  description: 'Analyze cognitive metrics and produce diagnoses with improvement hypotheses',
  input_schema: {
      ...
      diagnoses: {
        ...
        items: {
          ...
          properties: {
            ...
            root_cause: { type: 'string' as const, enum: ['parameter', 'logic', 'data', 'architecture'] },
            hypotheses: {
              type: 'array' as const,
              items: {
                type: 'object' as const,
                properties: {
                  type: { type: 'string' as const, enum: ['param_adjustment', 'logic_extension', 'new_stage', 'strategy_change'] },
                  description: { type: 'string' as const },
                ...
                required: ['type', 'description', 'target_service', 'expected_improvement', 'confidence'],
              },
            },
          },
          required: ['dimension', 'severity', 'description', 'root_cause', 'hypotheses'],
        },
      },
...
export class DiagnosisService {
  ...
  async analyze(snapshot: CognitiveSnapshot): Promise<Result<DiagnosisResult, DomainError>> {
  ...
  runRuleBasedDiagnosis(snapshot: CognitiveSnapshot): Diagnosis[] {
    ...
    if (dims.coherence.score < DIMENSION_THRESHOLDS.coherence) {
      ...
      if (dims.coherence.posture === 'unknown') {
        hyps.push({
          ...
          description: 'Run introspection on first boot to establish coherence baseline',
          target_service: 'NightlyService',
          target_method: 'run',
          expected_improvement: 'Coherence score reflects actual belief state instead of default 0.5',
          confidence: 0.9,
          logic_proposal: 'BootstrapService should trigger IntrospectionService.run("full") after seeding beliefs. This ensures coherence_score and posture are computed from day one.',
        });
      }
      ...
      if (dims.coherence.contradictions > 3) {
        hyps.push({
          ...
          expected_improvement: 'Reduce contradictions by catching them earlier',
          confidence: 0.6,
          param_changes: [
            { key: 'similarity.contradiction_content', from: this.config.get('similarity.contradiction_content'), to: Math.max(0.3, this.config.get('similarity.contradiction_content') - 0.05) },
          ],
        });
      }
      if (dims.beliefs.avg_confidence < 0.5) {
        hyps.push({
          ...
          confidence: 0.5,
          param_changes: [
            { key: 'decay.rate_normal', from: this.config.get('decay.rate_normal'), to: Math.max(0.001, this.config.get('decay.rate_normal') * 0.7) },
          ],
        });
      }
      diagnoses.push({
        ...
        hypotheses: [{
          ...
          param_changes: [{
            ...
            to: dims.calibration.overconfident
              ? Math.max(0.05, this.config.get('bayesian.learning_rate') - 0.05)
              : Math.min(0.8, this.config.get('bayesian.learning_rate') + 0.05),
          }],
        }],
        created_at: '',
      });
    ...
    if (dims.episodes.total >= 5 && dims.episodes.success_rate < 0.5) {
      diagnoses.push({
        ...
        hypotheses: [{
          id: '',
          type: 'logic_extension',
          description: 'Add failure pattern analysis to deliberation: before selecting an approach, check similar past episodes and avoid strategies that failed',
          target_service: 'DeliberationService',
          target_method: 'deliberate',
          expected_improvement: 'Higher success rate by learning from past failures',
          confidence: 0.6,
          logic_proposal: 'Before generating options, query recent failed episodes with similar intentions. Add failure patterns as negative constraints to the LLM deliberation prompt.',
    ...
    if (dims.knowledge.gaps_open > 10) {
      diagnoses.push({
        ...
        hypotheses: [{
          id: '',
          type: 'new_stage',
          description: 'Add a knowledge gap triage stage to nightly: auto-close stale gaps, merge duplicates, prioritize by dependency count',
          target_service: 'NightlyService',
          expected_improvement: 'Reduced gap count, better signal-to-noise in knowledge gaps',
          confidence: 0.7,
          logic_proposal: 'New nightly stage: iterate open gaps, close those older than 14 days with no linked intentions, merge gaps with >0.8 similarity, sort by impact * dependency_count.',
        }],
        created_at: '',
      });
    }
    ...
    if (dims.intentions.stale_count > 5) {
      diagnoses.push({
        ...
        hypotheses: [{
          ...
          description: 'Auto-suspend intentions stale > 7 days, auto-abandon > 14 days',
          target_service: 'IntentionStackService',
          target_method: 'autoAdopt',
          expected_improvement: 'Cleaner intention stack, reduced noise',
          confidence: 0.8,
          logic_proposal: 'In autoAdopt(), add sweep: intentions with status "recognized" and updated_at < 7d → suspend. status "suspended" and updated_at < 14d → abandon.',
        }],
        created_at: '',
      });
    }
    ...
    if (dims.world_model.freshness_hours > 12) {
      ...
      diagnoses.push({
        ...
        root_cause: dims.world_model.freshness_hours > 100 ? 'logic' : 'parameter',
        hypotheses: dims.world_model.freshness_hours > 100
          ? [{
            id: '',
            type: 'logic_extension' as const,
            description: 'Trigger world model rebuild on every pipeline run, not just nightly',
            target_service: 'CognitivePipelineService',
            target_method: 'processMessage',
            expected_improvement: 'World model always fresh during active use',
            confidence: 0.7,
            logic_proposal: 'After step 4 (auto-adopt), check if world model is stale. If so, trigger WorldModelService.build() as a non-blocking fire-and-forget.',
  ...
  private async runLlmDiagnosis(snapshot: CognitiveSnapshot): Promise<Result<Diagnosis[], DomainError>> {
    // Gather context
    const [historyResult, episodesResult, nightlyResult] = await Promise.all([
      this.db.query<CognitiveSnapshot>('SELECT * FROM cognitive_snapshot ORDER BY timestamp DESC LIMIT 3'),
      this.db.query<any>('SELECT * FROM episode ORDER BY created_at DESC LIMIT 20'),
      this.db.query<any>('SELECT * FROM nightly_run ORDER BY started_at DESC LIMIT 1'),
    ]);
    ...
    const configDump = this.config.getAll()
      .filter((p) => p.tunable)
      .map((p) => `${p.key}: ${p.value} [${p.min}-${p.max}]`)
      .join('\n');

    const userMessage = this.buildDiagnosisPrompt(snapshot, history, episodes, latestNightly, configDump);

    const llmResult = await this.llm.call<{ diagnoses: any[]; summary: string }>({
    ...
    const raw = llmResult.value.data;
  ...
  private buildDiagnosisPrompt(
    ...
    const episodeSummary = episodes.length > 0
    ...
    for (const d of llm) {
      ...
      if (existing) {
        ...
        if (d.root_cause === 'logic' || d.root_cause === 'architecture') {
          ...
          existing.hypotheses.push(...d.hypotheses.filter(
            (h) => !existing.hypotheses.some((eh) => eh.description === h.description),
          ));
        }
      } else {
        byDimension.set(d.dimension, d);
