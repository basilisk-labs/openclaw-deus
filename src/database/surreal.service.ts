...
export class SurrealService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleDestroy(): Promise<void> {
    for (const id of this.liveQueries) {
      try { await this.db.query(`KILL $id`, { id }); } catch {}
    }
  ...
  async create<T>(table: string, data: T): Promise<Result<T, DatabaseError>> {
    try {
      ...
      return err(new DatabaseError(`Create in ${table} failed: ${error}`, error));
    }
  }
  ...
  async batchUpdate(sql: string, vars?: Record<string, unknown>): Promise<Result<number, DatabaseError>> {
    try {
      const results = await this.db.query(sql, vars);
      const arr = Array.isArray(results) && results.length > 0 ? results[0] : [];
  ...
  async live(table: string, callback: (data: any) => void): Promise<Result<string, DatabaseError>> {
    try {
      ...
      return err(new DatabaseError(`LIVE SELECT on ${table} failed: ${error}`, error));
    }
  }
