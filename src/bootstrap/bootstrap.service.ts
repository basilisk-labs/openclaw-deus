...
export class BootstrapService {
  ...
  async validate(workspaceRoot?: string): Promise<Result<BootstrapReport, DomainError>> {
    ...
    checks.push({
      ...
      ok: pingResult.isOk(),
      detail: pingResult.isOk() ? 'SurrealDB connected' : `SurrealDB unreachable: ${pingResult.error.message}`,
    });
  ...
  async seedFromCoreJsonl(workspaceRoot?: string, force = false): Promise<Result<{ seeded: number; skipped: number }, DomainError>> {
    const root = workspaceRoot || process.cwd();
    const seedPath = path.join(root, BELIEFS_SEED_FILE);
    ...
    if (!force) {
      const existing = await this.db.query<Belief[]>('SELECT * FROM belief LIMIT 1');
    ...
    for (const line of lines) {
      try {
        ...
        else this.logger.warn(`Failed to seed belief ${belief.belief_id}: ${result.error.message}`);
      } catch (e) {
  ...
  async runMigrations(): Promise<Result<void, DomainError>> {
    ...
    if (!fs.existsSync(migrationsDir)) {
      return err(new FileSystemError('Migrations directory not found', migrationsDir));
    }
  ...
  async isSeeded(): Promise<Result<boolean, DomainError>> {
    const result = await this.db.query<Belief[]>('SELECT * FROM belief LIMIT 1');
    if (result.isErr()) return err(result.error);
  ...
  private validateSeedFile(seedPath: string): BootstrapCheck {
    if (!fs.existsSync(seedPath)) {
      return { name: 'seed:core.jsonl', ok: false, detail: `Seed file missing: ${seedPath}` };
    }
    ...
    try {
      ...
      return { name: 'seed:core.jsonl', ok: true, detail: `${valid} valid beliefs` };
    } catch (e) {
      return { name: 'seed:core.jsonl', ok: false, detail: `Parse error: ${e}` };
    }
  }
}
