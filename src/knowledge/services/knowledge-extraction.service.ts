...
import { Knowledge, KnowledgeExtractionResult } from '../../common/types/knowledge.types';
      ...
      new_knowledge: {
        type: 'array' as const, items: {
          type: 'object' as const, properties: {
            ...
            kind: { type: 'string' as const, enum: ['fact', 'inference', 'procedural', 'meta'] },
            domain: { type: 'string' as const },
            confidence: { type: 'number' as const },
            evidence_quality: { type: 'string' as const, enum: ['explicit_statement', 'strong_implication', 'behavioral_pattern', 'weak_inference'] },
            reasoning: { type: 'string' as const },
          },
          required: ['content', 'kind', 'domain', 'confidence', 'evidence_quality', 'reasoning'],
        },
      },
      updated_knowledge: {
        type: 'array' as const, items: {
          type: 'object' as const, properties: {
            ...
            update_type: { type: 'string' as const, enum: ['reinforced', 'revised', 'contradicted'] },
...
export class KnowledgeExtractionService {
  ...
  async extractFromInteraction(content: string): Promise<Result<KnowledgeExtractionResult, DomainError>> {
    // Skip trivial messages
    if (content.trim().length < 30) {
      return ok({ new_knowledge: [], updated_knowledge: [], knowledge_gaps: [] });
    }

    // Get existing knowledge for context
    const existing = await this.knowledge.findAll({ status: 'active' });
    const existingKnowledge = existing.isOk() ? existing.value.slice(0, 30) : [];
    ...
    const result = await this.llm.call<KnowledgeExtractionResult>({
      ...
      userMessage,
      tools: [EXTRACT_TOOL],
      forceTool: 'update_knowledge',
      cacheKey,
      cacheTtlMs: 4 * 3600_000,
    });

    if (result.isErr()) {
      this.logger.warn(`LLM knowledge extraction failed: ${result.error.message}`);
      return this.fallbackExtraction(content);
    }
  ...
  private async applyExtraction(extraction: KnowledgeExtractionResult, sourceContent: string): Promise<void> {
   ...
   */
  private fallbackExtraction(content: string): Result<KnowledgeExtractionResult, DomainError> {
    const result: KnowledgeExtractionResult = { new_knowledge: [], updated_knowledge: [], knowledge_gaps: [] };
