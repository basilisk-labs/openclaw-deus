    ...
    providers: [
      ...
      },
      {
        ...
        useValue: {
          evaluate: jest
            ...
            .mockReturnValue(
              ok({
                decision: dissensusDecision,
                trigger_type:
                  dissensusDecision === "allow" ? "none" : "invariant_conflict",
                reason: "test",
              }),
            ),
        },
      ...
      },
      {
        ...
        useValue: {
          ...
          predictGoalSuccess: jest
            .fn()
            .mockReturnValue({ success_probability: 0.5, blockers: [] }),
...
describe("DeliberationService", () => {
  ...
  describe("fallback mode output structure", () => {
    ...
    it("should list LLM unavailability as a risk", async () => {
      const result = await service.deliberate(mockIntention);
      const option = result._unsafeUnwrap().deliberation.options[0];
      expect(option.risks.some((r) => r.toLowerCase().includes("llm"))).toBe(
        true,
      );
    });
  });
